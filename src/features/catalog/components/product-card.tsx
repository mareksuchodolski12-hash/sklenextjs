...
export function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <article className="overflow-hidden rounded-2xl border border-brand-sage/20 bg-white shadow-soft">
      <div className={`relative flex aspect-[4/3] items-end bg-gradient-to-br p-4 ${accent}`}>
        <div className="rounded-full bg-white/85 px-3 py-1 text-xs uppercase tracking-[0.12em] text-brand-moss">
          {product.floweringSeasons.map((season) => formatLabel(season)).join(' · ')}
        </div>
        {product.badges.length > 0 ? (
          <div className="absolute left-3 top-3 rounded-full bg-brand-moss px-3 py-1 text-xs font-semibold uppercase tracking-wide text-brand-cream">
