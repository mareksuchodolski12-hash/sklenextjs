...
import { mockProducts, productCategories } from '@/data/mock/catalog';
import {
  applyCatalogFilters,
  hasActiveFilters,
...
export const metadata: Metadata = {
  ...
  description: 'Discover premium garden flowers and outdoor plants by style, season, and practical growing needs.',
};
...
export default async function PlantsPage({ searchParams }: PlantsPageProps) {
  ...
  const filtered = applyCatalogFilters(mockProducts, filters);
    ...
    <section className="py-12 sm:py-16">
      <Container className="space-y-8">
        <header className="space-y-4">
          ...
          <h1 className="font-serif text-4xl text-brand-moss sm:text-5xl">Shop outdoor flowers and garden plants</h1>
          <p className="max-w-3xl text-sm leading-relaxed text-brand-charcoal/75 sm:text-base">
            Browse by practical needs and visual mood to find plants that suit your space, light, and maintenance preferences.
          </p>
        </header>
        ...
        <div className="rounded-2xl border border-brand-sage/20 bg-white p-4 sm:p-5">
          ...
          {hasActiveFilters(filters) ? (
            <div className="mt-4 border-t border-brand-sage/20 pt-4">
              <ActiveFilterChips filters={filters} categories={productCategories} />
            </div>
          ) : null}
        </div>
        ...
        <div className="grid gap-8 lg:grid-cols-[280px_1fr] lg:items-start">
          <aside className="lg:sticky lg:top-24">
            <div className="hidden lg:block">
              <FilterPanel categories={productCategories} filters={filters} />
            </div>
            <details className="rounded-2xl border border-brand-sage/20 bg-white p-4 lg:hidden">
              <summary className="cursor-pointer list-none text-sm font-semibold text-brand-moss">Filters & price range</summary>
              <div className="mt-4">
                <FilterPanel categories={productCategories} filters={filters} />
              </div>
            </details>
...
function EmptyState({ filters }: { filters: CatalogFilterState }) {
  return (
    <div className="rounded-2xl border border-brand-sage/20 bg-white px-6 py-14 text-center">
      ...
      <h2 className="mt-3 font-serif text-3xl text-brand-moss">No plants match the current filters</h2>
      <p className="mx-auto mt-3 max-w-xl text-sm leading-relaxed text-brand-charcoal/75">
        Try broadening your criteria by removing a few filters or resetting the search to explore the full curated catalog.
      </p>
      <div className="mt-6 flex flex-wrap items-center justify-center gap-3">
        <Link href="/plants" className="rounded-full bg-brand-moss px-5 py-2 text-sm font-semibold text-brand-cream">
          Reset all filters
        </Link>
        <Link href={filters.sort === 'featured' ? '/plants' : `/plants?sort=${filters.sort}`} className="rounded-full border border-brand-sage/35 px-5 py-2 text-sm font-semibold text-brand-moss">
          Keep sort only
        </Link>
      </div>
    </div>
